...
impl<'a> Request for SurfRequest<'a> {
    ...
    async fn response(&self) -> Result<surf::Response> {
        ...
        let response = request.send().await;//.map_err(|e| Err(format!("Request failed with {}", e.to_string())));

        if cfg!(feature = "fail-on-err") && !response.unwrap().status().is_success() {
            return Err(anyhow!("Request failed with code {}", response.unwrap().status()));
        }
    ...
    async fn response_data(&self, etag: bool) -> Result<(Vec<u8>, u16)> {
        ...
        let body = response.body_bytes().await;//.map_err(|e| Err(format!("Request failed with {}", e.to_string())));
        let mut body_vec = Vec::new();
        let &mut r[..] = &mut body;
        body_vec.extend_from_slice(r);
        if etag {
